r"""Provide interpolation methods for the distribution function."""
...
def propagate_error(error: NDArray) -> float:
    r"""Propagate error for `binned_statistic_dd`."""
...
def process_chunk(  # noqa: PLR0913
    ...
    r"""Interpolate a ``data_chunk`` onto a regular grid.
...
def interpolate_distribution(
    ...
    r"""Interpolate a particle distribution function onto a regular grid.
    ...
    new_coordinates = {"time": da.time} | {
        item.name: np.arange(item.size) for item in grid.values()
    }
